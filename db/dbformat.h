...
// leveldb 中的每次更新(put/delete)操作都拥有一个版本，由 SequnceNumber 来标识，整个 db 有一个 全局值保存着当前使用到的 SequnceNumber
...
// db 内部操作的 key
// db 内部需要将 user key 加入元信息(ValueType/SequenceNumber)一并做处理
...
// 排序时，会先使用 user-comparator 比较 user-key，如果 user-key 相同，则比较 SequnceNumber，SequnceNumber 大的为小。
// 因为 SequnceNumber 在 db 中全局 递增，所以，对于相同的user-key，最新的更新(SequnceNumber 更大)排在前面，在查找的时候， 会被先找到。
...
class LookupKey {
 ...
 private:
  ...
  //    tag      uint64                               SequnceNumber/ValueType
  //                                    <-- end_
